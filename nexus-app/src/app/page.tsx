...
export default function LandingPage() {
  return (
    <div className="min-h-screen bg-ink text-white selection:bg-acid selection:text-black">
      <div className="mx-auto max-w-[1600px] px-5 md:px-8">
        ...
        <header className="flex flex-wrap items-start justify-between gap-6 py-6 md:py-8">
          ...
          <nav className="flex items-center gap-2">
            <span
              aria-hidden
              className="flex h-11 w-11 items-center justify-center rounded-full bg-white/[0.06] text-white/60"
            >
              —
            </span>
            <Link
              href="/auth"
              className="group flex h-11 items-center gap-2.5 rounded-full bg-white px-6 text-[11px] font-bold uppercase tracking-[0.16em] text-ink transition-colors hover:bg-white/85"
            >
              Bắt đầu
            ...
            <Link
              href="/dashboard"
              className="flex h-11 items-center rounded-full bg-white/[0.06] px-6 text-[11px] font-bold uppercase tracking-[0.16em] transition-colors hover:bg-white/[0.12]"
            >
              Bảng điều khiển
            </Link>
          </nav>
        ...
        {/* ---------- DẢI CHÂN MÀN HÌNH ---------- */}
        <div className="relative flex items-center justify-between border-t border-white/10 py-5">
          {['a', 'b', 'c', 'd'].map((k) => (
            <span key={k} aria-hidden className="text-sm leading-none text-white/25">
              +
            </span>
          ))}
          <span className="absolute left-1/2 -translate-x-1/2 text-[10px] font-bold uppercase tracking-[0.3em] text-white/45">
            Cuộn để xem tiếp
          </span>
        </div>
      </div>
      ...
      <div className="mx-auto max-w-[1600px] px-5 md:px-8">
          ...
          <Reveal>
            <div className="flex flex-wrap items-end justify-between gap-6 border-b border-white/15 pb-8">
              <h2 className="max-w-2xl text-5xl font-black uppercase leading-[1.18] tracking-[-0.04em] md:text-7xl">
                Ba bước,
                <br />
                <span className="nx-outline-text">không hơn</span>
              </h2>
          ...
          <div className="mt-10 grid grid-cols-1 gap-5 md:grid-cols-3">
            {STEPS.map((step, i) => (
              <Reveal key={step.index} delay={i * 0.08}>
                <article className="nx-lift flex h-full flex-col justify-between rounded-[24px] bg-white-panel/50 border border-white/10 text-white p-8 md:p-10">
                  <div>
                    ...
                    <p className="mt-4 text-[15px] leading-relaxed text-white/60">{step.body}</p>
                  </div>
                  <p className="mt-12 border-t border-white/10 pt-5 text-[11px] font-bold uppercase tracking-[0.2em] text-white/45">
                    {step.meta}
                  </p>
                </article>
              </Reveal>
        ...
        <section className="pb-32 md:pb-44">
          <Reveal>
            <div className="relative overflow-hidden rounded-[32px] bg-ink-panel border border-white/10 px-8 py-24 text-center md:py-32">
              ...
              <div className="relative">
                ...
                <h2 className="mt-8 text-[11vw] font-black uppercase leading-[1.14] tracking-[-0.045em] text-white md:text-[8rem]">
                  Bắt tay
                  <br />
                ...
                <Link
                  href="/auth"
                  className="group mt-14 inline-flex h-16 items-center gap-4 rounded-full bg-acid px-12 text-[11px] font-bold uppercase tracking-[0.2em] text-white transition-all hover:gap-6 hover:bg-white hover:text-ink"
                >
                  Vào Nexus
